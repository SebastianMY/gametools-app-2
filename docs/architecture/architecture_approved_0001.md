...
### ADR-9: Fixed Score Increment Buttons (+1, +5, +10)

**Decision:** Score tracking uses three fixed increment buttons: **+1, +5, +10**. No per-game customization or settings menu in v1.

**Context:**
- These three increments cover the vast majority of board games (Scrabble, Cribbage, Darts, Yahtzee, Catan, etc.)
- Reduces complexity: no per-game settings system needed in v1
- Query 1 resolved: spec does not mandate custom increments; fixed buttons are appropriate for MVP
- Users accustomed to board games intuitively understand these increments

**Alternatives Considered:**
1. **Per-game customization** – More flexible but requires a settings screen, adds state complexity, defers MVP launch
2. **Calculator mode** – Allow freeform entry; conflicts with touch-based quick gameplay
3. **Contextual increments** – Button values change per game (e.g., Scrabble shows +25 for bonus); too complex for v1

**Trade-offs:**
- **Accepted:** Games with non-standard scoring (e.g., some trick-taking games needing +2 or +15) require users to tap multiple times
- **Gained:** Minimal UI, instant gameplay, no onboarding friction

**Status:** Accepted

---

### ADR-10: Optional Game Names with Auto-Generated Labels

**Decision:** Game names are optional. If a user skips naming, the game is labeled with **"Game from [Month Day, Year]"** (e.g., "Game from March 19, 2026").

**Context:**
- Query 2 resolved: spec does not mandate custom names; optional reduces friction
- Most casual game sessions don't need memorable names; timestamp context is sufficient
- Auto-generated label distinguishes games chronologically without extra user input
- Reduces dialog fatigue on game creation

**Alternatives Considered:**
1. **Mandatory names** – Forces users to think of a name every session; friction for quick games
2. **No names, ID only** – Confusing if user creates multiple games same day; no human-readable context
3. **Name + timestamp** – Best of both, but requires database schema supporting both; optional name achieves same effect

**Trade-offs:**
- **Accepted:** Auto-generated labels are less meaningful than user-chosen names (e.g., "Friday Poker Night"). If multiple games created same day, labels are similar
- **Mitigation:** Add time-of-day to label in v1.1 if user feedback shows confusion
- **Gained:** Fast game creation, reduced cognitive load

**Status:** Accepted

---

### ADR-11: No Undo Functionality in v1

**Decision:** No undo/redo feature. Users correct scores manually via the decrement button (−1, −5, −10).

**Context:**
- Query 3 resolved: spec does not mandate undo; simplifies state management
- Undo requires maintaining a score history stack, adding storage overhead and UI complexity
- Most casual board game apps (Catan Universe, Cribbage Classic) do not include undo
- Manual correction is sufficient for typical use case (accidental tap)
- Keeps `GameStateContext` focused on current game state, not history

**Alternatives Considered:**
1. **Full undo/redo stack** – Convenient but requires history tracking, more memory, and UI controls (Undo, Redo buttons)
2. **Single-level undo** – Compromise; still requires history tracking
3. **Manual correction only** – Simple, matches current decision

**Trade-offs:**
- **Accepted:** Users must live with mistakes; no "undo last 5 taps" convenience
- **Mitigation:** Large increment/decrement buttons (NFR-U2 touch target size) and clear labeling minimize accidental taps
- **Gained:** Simpler state, faster performance, smaller storage footprint

**Status:** Accepted

---

### ADR-12: Maximum 8 Simultaneous Touches in Lottery Feature

**Decision:** Lottery feature enforces a **hard limit of 8 simultaneous touches**. If a 9th finger touches, display a visual constraint message and ignore the 9th touch.

**Context:**
- Query 4 resolved: spec explicitly states "typically 2–8 players"; 8 is a reasonable, stated boundary
- Gesture Handler reliably tracks up to 10 fingers; 8 is safe
- Enforcing a constraint simplifies UI (no need to render 9+ circles or logic for arbitrary N)
- Real-world board games have 2–6 players; 8 covers all reasonable edge cases

**Alternatives Considered:**
1. **No limit (allow N fingers)** – Overly complex; UI breaks with 10+ circles
2. **Smaller limit (4 fingers max)** – Too restrictive; excludes valid 5–8 player games
3. **Soft limit (warn but allow overflow)** – Confusing; undefined behavior if too many touches

**Trade-offs:**
- **Accepted:** Large groups (9+ players) cannot use Lottery feature; they use Score feature instead. Acceptable given rarity
- **Gained:** Clean UI, predictable behavior, simplified state tracking

**Status:** Accepted

---

### ADR-13: d6 (Six-Sided Dice) Only in v1

**Decision:** Dice feature supports **d6 only** (standard six-sided die). No d4, d8, d10, d12, or d20 in v1.

**Context:**
- Query 5 resolved: spec does not mandate multi-sided dice; d6 is most common in board games (Yahtzee, Catan, Farkle, Cribbage, Backgammon)
- Restricts animation logic to one template, display to numbers 1–6, algorithm to single RNG call
- RPG-focused apps are out-of-scope; board game focus justifies d6-only MVP
- Extension to other dice types is straightforward in v2 (die type selector, new animation variants)

**Alternatives Considered:**
1. **All standard dice (d4–d20)** – RPG coverage but adds animation complexity, display logic, and testing burden
2. **d6 and d20 only** – Splits difference but still adds complexity for RPG edge case
3. **d6 only** – Focused, simple, covers ~85% of board game use cases

**Trade-offs:**
- **Accepted:** D&D/Pathfinder players cannot roll d20s; out-of-scope for this app. Users needing d20 use another app
- **Gained:** Minimal animation logic, simple display, fast first release

**Status:** Accepted

---

### ADR-14: Single Standard Haptic Feedback Pattern

**Decision:** Haptic feedback on Lottery countdown completion uses a **single pattern: 0.1-second vibration at medium intensity** (Expo's `Vibration.vibrate(100)`). No customization menu in v1.

**Context:**
- Query 6 resolved: spec requires haptic feedback (FR-L10) but does not mandate customization
- Single pattern is sufficient for tactile feedback and celebration moment
- Settings menu is out-of-scope for v1 (adds navigation, state, testing)
- Expo's `Vibration.vibrate()` is simple, reliable, works on iOS and Android

**Alternatives Considered:**
1. **Customizable vibration (settings menu)** – More user control but requires settings infrastructure, defers v1 launch
2. **Multiple patterns (short/medium/long)** – No requirement; adds UI complexity
3. **No vibration** – Fails FR-L10 requirement

**Trade-offs:**
- **Accepted:** Users cannot mute or customize vibration intensity via in-app settings
- **Mitigation:** Device-level vibration settings apply (users can silence phone in settings); acceptable
- **Gained:** Simple implementation, faster launch, consistent experience

**Status:** Accepted

---

### ADR-15: Graceful Fallback for Deleted Last Session Game

**Decision:** On app reopen, if the last active game ID is not found in the games list, **fall back to Score Home screen** (list of all games) and display a toast message: **"Last game was deleted; here are your saved games."**

**Context:**
- Query 7 resolved: prevents ghost data when users delete the last active game
- Game ID is a foreign key; if game doesn't exist, reference is broken
- Graceful fallback avoids crashes and confusion
- Toast explains what happened without alarming the user
- Keeps persistence logic simple: store and retrieve ID, not entire game object

**Alternatives Considered:**
1. **Restore full game object** – More robust but violates single source of truth; inconsistency if game object and games list diverge
2. **Crash/error screen** – Poor UX; users feel app is broken
3. **Empty state** – No explanation; users confused about where their games went

**Trade-offs:**
- **Accepted:** User loses convenience of resuming last game if they deleted it
- **Mitigation:** Improve delete confirmation dialog with warning ("This will be lost when app restarts" or similar)
- **Gained:** Robust error handling, clear communication, simple persistence logic

**Status:** Accepted

---

### ADR-16: Landscape Orientation Support on All Screens

**Decision:** All three feature screens (Dice, Score, Lottery) support landscape and portrait orientation. Use `useWindowDimensions()` hook to adapt layouts dynamically.

**Context:**
- Query 8 resolved: spec requirement NFR-C3 ("works on phones and tablets") implicitly includes landscape
- Modern mobile apps support all orientations; users expect landscape to work
- Tablets especially benefit (larger screens, frequently held landscape)
- Expo/React Native abstracts iOS and Android orientation differences
- Responsive layouts (width-based conditional rendering) simpler than fighting OS

**Alternatives Considered:**
1. **Portrait-only** – Limits use case for tablets; poor UX on iPad
2. **Selective landscape (some screens only)** – Inconsistent; confuses users
3. **All orientations, responsive layout** – Current decision; best UX

**Trade-offs:**
- **Accepted:** Requires testing on portrait and landscape; slightly more complex layout logic
- **Accepted:** Some screens may feel cramped in portrait on small phones (e.g., 5" screen with large buttons)
- **Mitigation:** Use `useWindowDimensions()` to conditionally adjust padding, font sizes, button sizing
- **Gained:** Works on tablets, supports user preferences, modern expectations

**Status:** Accepted

---

### ADR-17: Equal WCAG AA Accessibility Standards for All Features

**Decision:** All three feature screens (Dice, Score, Lottery) must pass WCAG AA compliance equally. No feature is exempt or lower priority.

**Context:**
- Query 9 resolved: spec explicitly requires WCAG AA (NFR-A1); no ambiguity or exemptions
- All features are primary user-facing surfaces; inconsistent accessibility is poor UX
- Board games benefit all users, including those with low vision, color blindness, motor impairments
- Implementing AA from the start is cheaper than retrofitting post-launch

**Alternatives Considered:**
1. **WCAG A (baseline minimum)** – Fails spec requirement (AA mandated)
2. **Tiered accessibility (AA for Score, A for Dice/Lottery)** – Creates inconsistency; conflicts with spec
3. **No accessibility** – Fails spec; excludes users with disabilities

**Trade-offs:**
- **Accepted:** Feature development must include accessibility testing from day one (screen reader testing, color contrast checks, touch target sizing)
- **Accepted:** Adds QA effort; longer feature development cycles
- **Gained:** Inclusive product, meets legal requirements, better UX for all users

**Status:** Accepted

---

### ADR-18: Responsive Layout Scaling for v1; Defer Tablet Optimization to v2

**Decision:** v1 uses a **single responsive layout** that adapts font sizes and padding based on screen width (`useWindowDimensions()`). Conditional layouts for large screens (>600dp width) are minimal. Full tablet-optimized layouts (e.g., split-screen Score list + current game) defer to v2.

**Context:**
- Query 10 resolved: spec requires tablet support (NFR-C3) but does not mandate tablet-specific layouts
- Single layout simplifies codebase, reduces testing matrix, speeds v1 launch
- Responsive scaling via conditional padding/font sizes handles phones (5"–6") and most tablets (7"–10") adequately
- Majority of users are phone-based; tablets secondary use case
- Shipping faster with responsive scaling is preferable to delaying for perfect tablet layouts

**Alternatives Considered:**
1. **Phone-only layout** – Fails tablet requirement; poor UX on iPad
2. **Full tablet layouts (split-screen, etc.)** – Better UX but delays v1; more testing; more code branches
3. **Responsive scaling + minimal conditional layouts** – Current decision; balanced approach

**Trade-offs:**
- **Accepted:** Tablets may have unused whitespace (e.g., wide margins in landscape) or cramped layouts (portrait on small tablets)
- **Accepted:** No split-screen or dedicated tablet features in v1
- **Mitigation:** Test on tablet simulators; adjust padding and font scaling based on feedback
- **Gained:** Faster launch, simpler codebase, easier maintenance
